  ...
  return (
    <main className="container-xl">
      ...
      <nav className="mx-auto flex max-w-[730px] flex-wrap items-center justify-between gap-x-4 gap-y-6 border-t border-gray-200 pt-8">
        {i < allPosts.length - 1 && (
          <Link href={`/${allPosts[i + 1].slug}`} className="group flex flex-col">
            <span className="ml-7 text-xs text-slate-500">Previous</span>
            <div className="mt-1 flex items-center font-semibold">
              <ChevronRightIcon className="mr-1 h-6 w-6 rotate-180 fill-slate-500" />
              <span className="line-clamp-1 transition-colors group-hover:text-slate-500">
                {allPosts[i + 1].title}
              </span>
            </div>
          </Link>
        ...
        {i > 0 && (
          <Link href={`/${allPosts[i - 1].slug}`} className="group ml-auto flex flex-col">
            <span className="ml-auto mr-7 text-xs text-slate-500">Next</span>
            <div className="mt-1 flex items-center font-semibold">
              <span className="line-clamp-1 transition-colors group-hover:text-slate-500">
                {allPosts[i - 1].title}
              </span>
